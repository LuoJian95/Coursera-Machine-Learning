...
ylabel('Cost J');
s = 'alpha:';
legend(strcat(s,num2str(alpha)),strcat(s,num2str(alpha2)),strcat(s,num2str(alpha3)))
...
fprintf(['Predicted price of a 1650 sq-ft, 3 br house ' ...
         '(using gradient descent with alpha: %f)\n $%f\n'],alpha,price);
fprintf(['Predicted price of a 1650 sq-ft, 3 br house ' ...
         '(using gradient descent with alpha: %f)\n $%f\n'],alpha2,price2);
fprintf(['Predicted price of a 1650 sq-ft, 3 br house ' ...
         '(using gradient descent with alpha: %f)\n $%f\n'],alpha3,price3);
...
test = [1650 3];
%test = (test-mu)./sigma;  正规方程不需要数据预处理！！！
test = [ones(size(test,1), 1),test]; %但还是需要增加第一列全1 因为计算预测的时候要乘上theta_0

price = test * theta_normalEqn;
